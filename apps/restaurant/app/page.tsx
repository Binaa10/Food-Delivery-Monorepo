...
export default function Page() {
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_right,_#052e16_0%,_#020617_45%,_#020617_100%)] px-6 py-10 text-zinc-50">
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-7">
        <section className="rounded-3xl border border-emerald-200/10 bg-white/[0.03] p-7 shadow-[0_20px_80px_rgba(16,185,129,0.2)] backdrop-blur">
          ...
          <div className="mt-3 flex flex-wrap items-end justify-between gap-4">
            <div className="space-y-2">
              ...
              <p className="max-w-2xl text-zinc-300">
                Mark orders ready and track real-time stats synced with the customer app.
              </p>
            </div>
            <SharedButton disabled={loading} onClick={() => void markReady()}>
        ...
        <section className="grid gap-4 sm:grid-cols-3">
          <article className="rounded-2xl border border-white/10 bg-zinc-900/60 p-4">
            ...
            <p className="mt-1 text-3xl font-bold tabular-nums text-emerald-300">{totalOrders}</p>
          </article>
          <article className="rounded-2xl border border-white/10 bg-zinc-900/60 p-4">
            <p className="text-xs uppercase tracking-wider text-zinc-400">Active apps</p>
            <p className="mt-1 text-3xl font-bold tabular-nums text-cyan-300">{rows.length}</p>
          </article>
          <article className="rounded-2xl border border-white/10 bg-zinc-900/60 p-4">
            <p className="text-xs uppercase tracking-wider text-zinc-400">Top app</p>
            <p className="mt-1 truncate text-xl font-semibold text-yellow-300">
        ...
        <section className="overflow-hidden rounded-3xl border border-white/10 bg-zinc-900/70 shadow-2xl">
          <div className="border-b border-white/10 px-5 py-4">
            <h2 className="text-lg font-semibold">Live order stats</h2>
                ...
                {leaderboard.length === 0 ? (
                  <tr>
                    <td colSpan={2} className="px-5 py-8 text-center text-zinc-500">
                      No stats yet - mark an order to seed the table.
                    </td>
                  </tr>
                ) : (
                  leaderboard.map((r) => (
